...
class OrderPage(BasePage):
    ...
    #'Кликаем на Кнопку Заказать вверху главной страницы'
    def click_order_button_header(self):
        self.click_element(BasePageLocators.ORDER_BUTTON_HEADER)
    
    #'Кликаем на Кнопку Заказать внизу главной страницы'
    ...
    # Выбираем станцию метро: {subway_name}
    ...
    # Клик вне области попапа Календарь
    def click_outside_calendar(self):
        actions = ActionChains(self.driver)
        actions.move_by_offset(10, 10).click().perform()

    #'Кликаем на Заказать в Форме заказа'
    ...
    #'Подтверждение заказа. Кликаем кнопку Да'
   ...
   #'Проверяем наличие окна с информацией о заказе'
    ...
    @allure.step("Вводим Имя: {name}, Фамилия: {surname}, Адрес: {address},  Станцию метро: {subway_name}, Телефон: {phone}.")
    ...
    @allure.step("Вводим Дату, когда привезти самокат: {date}, выбираем период аренды: {period}, Цвет самоката: {colour},  Комментарий для курьера: {comment}.")
